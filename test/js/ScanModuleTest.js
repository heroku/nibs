...
describe('ScanCtrl', function() {
  ...
  var nonQRImage =  {cancelled: false, format: 'NO'};
  ...
  describe("takePicture()", function() {
    ...
    it("shouldn't do anything when the scan's not a QR code", inject(function($controller) {
      var scope = {};
      ...
      sinon.stub(cordovaBarcodeScanner, "scan").returns(fakePromise(nonQRImage));
    ...
    it("should warn of an invalid coupon", inject(function($controller) {
      var scope = {};

      Coupon.should.exist;
      ionicPopup.should.exist;

      var alertStub = sinon.stub(ionicPopup, "alert");

      var couponStub = sinon.stub(Coupon, "check");
      couponStub.returns(fakePromise({data: {valid: false, name: "name", description: "desc"} } ) );
      ...
      couponStub.calledOnce.should.be.true;
      couponStub.firstCall.args[0].should.eql(couponInfo);
      couponStub.calledBefore(alertStub).should.be.true;

      alertStub.calledOnce.should.be.true;
      alertStub.firstCall.args[0].should.eql({title: "Invalid coupon", templateUrl: "templates/invalid-coupon.html", subTitle: "name", scope: scope});
    ...
    it("shouldn't consume the coupon without confirmation", inject(function($controller) {
      var scope = {};
      ...
      var confirmStub = sinon.stub(ionicPopup, "confirm");
      confirmStub.returns(fakePromise(false));
      ...
      confirmStub.calledOnce.should.be.true;
      confirmStub.firstCall.args[0].should.eql({subTitle: "name", template: "desc", title: "Valid coupon"});
      confirmStub.calledBefore(toastStub).should.be.true;

      couponStub2.called.should.be.false;

      toastStub.calledOnce.should.be.true;
      toastStub.firstCall.args[0].should.eql('Coupon use cancelled');
    }));
